...
import com.example.think.waterworksapp.adapter.ImageSlectAdapter;
import com.example.think.waterworksapp.bean.FolderBean;
import com.example.think.waterworksapp.dialog.DialogManager;
import com.example.think.waterworksapp.popup_window.SelectImgDirPopupWindow;
import com.example.think.waterworksapp.utils.MyFileNameFileter;
import com.example.think.waterworksapp.utils.MyThreadPool;
...
//        System.out.println(currentDir);
    ...
    private void initDatas() {
        currentImageListSize = getIntent().getIntExtra(EquipmentInformationActivity.CURRENT_IMAGE_LIST_LENGTH,0);
        //遍历所有的图片
        if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            dialog.showRecordingDialog("当前储存卡不可用");
            return;
        }
        if (pool == null) {
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.img_select_commit_btn:
                ...
                if((currentImageListSize+dirPaths.size())<17) {
                    ...
                    dialog.showRecordingDialog("图片上传数目不得大于16张");
                }
                break;
            default:
                break;
